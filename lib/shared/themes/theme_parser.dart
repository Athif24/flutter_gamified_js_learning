import 'dart:math' show cos, sin, pow, pi;
...
const daisyRawThemes = <String, Map<String, String>>{
  'light': {
    'color-scheme': 'light',
...
// ════════════════════════════════════════════════════════════════════════════
// THEME DISPLAY NAMES   &   EMOJIS
// ════════════════════════════════════════════════════════════════════════════

const _themeNames = <String, String>{
  'light': 'Light',
  'cupcake': 'Cupcake',
  'bumblebee': 'Bumblebee',
  'emerald': 'Emerald',
...
const _themeEmojis = <String, String>{
  'light': '☀️',
  'cupcake': '🧁',
  'bumblebee': '🐝',
  'emerald': '💎',
...
String _themeName(String id) => _themeNames[id] ?? id;
String _themeEmoji(String id) => _themeEmojis[id] ?? '🎨';

// ════════════════════════════════════════════════════════════════════════════
// OKLCH → FLUTTER COLOR   (pure Dart, no external deps)
// ════════════════════════════════════════════════════════════════════════════

Color oklchToColor(String raw) {
  final s = raw.trim();
  assert(s.startsWith('oklch(') && s.endsWith(')'), 'Invalid oklch: $s');
  final inner = s.substring(6, s.length - 1);
  final parts = inner.split(' ');
  final l = double.parse(parts[0].replaceFirst('%', '')) / 100;
  final c = double.parse(parts[1]);
  final h = double.parse(parts[2]) * (pi / 180);

  final aVal = c * cos(h);
  final bVal = c * sin(h);

  final lM = l + 0.3963377774 * aVal + 0.2158037573 * bVal;
  final mM = l - 0.1055613458 * aVal - 0.0638541728 * bVal;
  final sM = l - 0.0894841775 * aVal - 1.2914855480 * bVal;

  final lCubed = lM * lM * lM;
  final mCubed = mM * mM * mM;
  final sCubed = sM * sM * sM;
  ...
  var b = -0.0041960863 * lCubed - 0.7034186147 * mCubed + 1.7076147010 * sCubed;
  ...
  return Color.from(
    alpha: 1.0,
    red: (r * 255).round().clamp(0, 255) / 255,
    green: (g * 255).round().clamp(0, 255) / 255,
    blue: (b * 255).round().clamp(0, 255) / 255,
  );
}

double _srgbTransfer(double c) {
...
BloomTheme daisyToBloomTheme(String id, Map<String, String> data) {
  ...
  final base300 = oklchToColor(data['--color-base-300']!);
  final baseContent = oklchToColor(data['--color-base-content']!);
  final primary = oklchToColor(data['--color-primary']!);
  final primaryContent = oklchToColor(data['--color-primary-content']!);
  final secondary = oklchToColor(data['--color-secondary']!);
  final secondaryContent = oklchToColor(data['--color-secondary-content']!);
  final accent = oklchToColor(data['--color-accent']!);
  final accentContent = oklchToColor(data['--color-accent-content']!);
  final neutral = oklchToColor(data['--color-neutral']!);
  final success = oklchToColor(data['--color-success']!);
  final warning = oklchToColor(data['--color-warning']!);
  final error = oklchToColor(data['--color-error']!);
  final info = oklchToColor(data['--color-info']!);

  return BloomTheme(
    id: id,
    name: _themeName(id),
    emoji: _themeEmoji(id),
    isLight: isLight,
    bgPrimary: base200,
    bgSurface: base100,
    bgSurface2: _deriveSurface2(base200, isLight),
    bgSurface3: base300,
    border: neutral,
    primary: primary,
    primaryContent: primaryContent,
    accent: accent,
    accentDark: _darken(accent, 0.2),
    accentText: accentContent,
    accentContent: accentContent,
    textPrimary: baseContent,
    textSecondary: baseContent.withValues(alpha: 0.7),
    textHint: baseContent.withValues(alpha: 0.5),
    mutedText: baseContent.withValues(alpha: 0.6),
    secondary: secondary,
    secondaryContent: secondaryContent,
    success: success,
    successText: success,
    successContent: baseContent,
    warning: warning,
    warningText: warning,
    warningContent: warning,
    error: error,
    errorText: error,
    errorContent: baseContent,
    info: info,
    infoText: info,
    infoContent: info,
  );
...
List<BloomTheme> buildBloomThemeList() {
  return daisyRawThemes.entries.map((e) => daisyToBloomTheme(e.key, e.value)).toList();
}
